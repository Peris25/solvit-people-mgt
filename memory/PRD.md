...
## Changelog (recent)
- **2026-02-19** — **Fresh-start reset for production handoff.**
  • `seed_data.py` reduced to 2 canonical admin accounts: **HR Admin (`jmining@solvit.co.ke`)** and **IT Admin (`support@solvit.co.ke`)** — both still using `DEMO_SEED_PASSWORD`. Demo employees, solvers, reviews, recognitions, candidates, stay interviews, pay-band alerts, exit interviews are no longer seeded.
  • New `purge_legacy_demo_data()` runs on every boot — idempotently deletes every record under `tenant_id=solvit` that isn't tied to one of the two canonical emails (across 30+ transactional collections). Re-runnable safely.
  • Login page (`Login.js`): removed Quick Demo Login block + `DEMO_USERS` array. Email placeholder updated to generic `you@solvit.co.ke`. Bottom CTA now reads "Need access? Contact your IT Administrator."
- **2026-02-19** — **Organogram view.** New `GET /api/employees/organogram` returns a tree of nodes (id, name, role_title, department, lifecycle_state, work_email, profile_photo_url) wired by `line_manager_id`. New `Organogram` page + `OrgNode` recursive component (split into its own file to dodge the `visual-edits` babel plugin's traversal limit; uses `React.createElement` for the self-referencing recursion). Includes per-node colour-coded lifecycle stripe, expand/collapse with direct-report count, and a live name/role/department/email search that prunes branches. Wired into sidebar for ALL roles.
- **2026-02-19** — **Sender.net email provider** added as a production option alongside Office 365 SMTP. `email_service.py` now branches on `provider` (`smtp` | `sender_net`); Sender.net uses the HTTP transactional API (`POST https://api.sender.net/v2/email/send`) with Bearer auth. UI: `EmailDelivery.js` adds a Provider dropdown on the Production card that switches the form between SMTP fields and a single API-key field. Same provider branching wired into the "Send Test Email" path. Masked secret handling extended to cover `api_key` (same `***` policy as `password`).
- **2026-02-14** — Reminder Service (41 rules across 11 groups), Peer Recognition dropdown, system-wide email-body audit (all in earlier entries above).
  • `backend/reminders/engine.py` — APScheduler-backed engine, timezone Africa/Nairobi, master enable/disable, per-rule enable/disable, configurable daily run time. Engine NEVER sends email directly — it always fires events into the Notification Service via `email_triggers.fire_and_forget`.
  • `backend/reminders/rules.py` — 41 rules with condition functions (Onboarding 5, Probation 3, Performance 6, Surveys 3, Leave 4, Recognition 5, Policy 2, Retention 3, Exit 3, Recruitment 2, System 5). All condition fns are defensive — schema-gap collections return [] gracefully instead of crashing.
  • `backend/routes/reminders.py` — `/api/reminders/{config,rules,log,runs,summary}` + `PUT /rules/{id}` toggle + `POST /rules/{id}/run-now`. IT Admin can edit, HR Admin can view.
  • `reminder_log` & `reminder_runs` collections with unique index on `(rule_id, dedup_key)` so duplicate fires are atomically prevented. 6 dedup scopes supported: milestone, cycle, weekly, every-N-days, daily, per-year, per-quarter, per-record.
  • 28 new `reminder.*` email templates added to the catalogue; engine reuses 13 existing onboarding / leave / performance / recognition / policy / retention templates.
  • Frontend: `ReminderRules.js` component (table, filter, group select, master pause/resume, daily-time setter, Run Now, log modal with runs + fired list + dedup keys). Wired into Settings as a new "Reminder Rules" tab.
  • pytest `tests/test_reminder_service.py` — verifies fire path, dedup skip, empty-match cleanliness.
  • Audit `tests/audit_reminder_templates.py` — all 28 reminder templates render with documented merge_tags.
- **2026-02-14** — Peer Recognition Nomination form: free-text input replaced with `EmployeePicker` dropdown. New `GET /api/employees/directory` endpoint exposes minimal-info directory to all authenticated users.
- **2026-02-14** — Recognition email body bug + system-wide email-body audit. 13 templates fixed; AST-based `audit_email_bodies.py` catches any future caller/template merge-tag mismatch. All 30 wired email templates render fully populated.
