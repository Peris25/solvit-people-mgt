...
- **2026-02-14** — System-wide email-body audit. Built `backend/tests/audit_email_bodies.py` — an AST-based check that, for every template stored in MongoDB, locates every code site that fires it (`fire_and_forget`, `trigger`, `send_email`, including dynamic `template_key = mapping.get(...)` patterns) and compares the supplied merge tags against the body's `{{placeholders}}`. Audit caught and fixed real blank-render bugs in: `onboarding.welcome`, `performance.review_complete`, `performance.pip_initiated`, `performance.pip_escalate`, `policy.published`, `retention.stay_interview_invite`, `lnd.project_completed`, `solver.activation`, `solver.tier_upgrade`, `solver.tier_downgrade`, `solver.suspension`, and all four `disciplinary.*` templates. **All 30 wired templates now render with every placeholder filled.** 40 seeded templates remain unwired (intentionally — placeholders for future flows).
